 ...
 */
    ...
    func postToServerFunction(){
        ...
        if(password == confirmPassword){
            var request = URLRequest(url: URL(string: "http://54.241.146.216/CSPhp/Register.php")!)
    ...
    func emailFunction(){
        ...
        email = emailField.text!
            ...
            var request = URLRequest(url: URL(string: "http://54.241.146.216/CSPhp/SendEmail.php")!)
    ...
    @IBAction func registerButton(_ sender: Any) {
        ...
        if accepted && overEighteen{
            self.registerOut.isEnabled = false
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
                ...
                if self.responseString == "Success" { //put if username = username from db and password = password from db then it does segue.
                   ...
                   // self.performSegue(withIdentifier: "howtoplaySegue", sender: self)
                    ...
                    let alert=UIAlertController(title: "Oops!", message: self.responseString, preferredStyle: UIAlertControllerStyle.alert);
                    alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.cancel, handler: nil));
                    //show it
                    self.show(alert, sender: self);
                    
                    //// enable the register button again
                    self.registerOut.isEnabled = true
                }
